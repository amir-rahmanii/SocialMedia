...
function UpdatePassword() {
    ...
    const { mutate: updatePass, isLoading } = usePostData('users/update-password',
        ...
        <Auth>
            <div className="bg-white dark:bg-black border dark:border-gray-300/20 border-gray-300 flex flex-col gap-2 p-4 pt-10 mt-16">
                ...
                <form onSubmit={e => e.preventDefault()} className="flex flex-col justify-center items-center gap-3 m-3 md:m-8">
                    ...
                    {isSuccessMyInfo && (
                        <Link to={`/profile/${myInfo?._id}`} className="text-sm font-medium text-blue-800">Back to Profile Page</Link>
                    )}
                </form>
            </div>
        </Auth >
